...
@synthesize expressionPatterns, expressionSubPatterns, cardTypes;
static PMCreditCardTypeParser *sharedInstance;


- (id)init {
    if (self = [super init]) {
        self.cardTypes = [self getActiveCards];
        [self ccChecksInit]; //init ccChecks
    }
    return self;
}
...
-(void)ccChecksInit {
	...
	NSString* path = [[NSBundle mainBundle] pathForResource:@"Mixtable-CardsData"
													 ofType:@"plist"];
	...
	for (NSString* cardType in cardTypes) {
		self.ccChecks = [NSMutableArray arrayWithCapacity:[cardTypes count]];
